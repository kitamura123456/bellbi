...
<div style="margin-bottom: 24px; display: flex; justify-content: space-between; align-items: center;">
    <h1 style="margin: 0; font-size: 24px; font-weight: 700; color: #5D535E; letter-spacing: 0.3px; font-family: 'Hiragino Sans', 'Yu Gothic', 'Meiryo', sans-serif;">商品編集</h1>
...
">
    ...
    <form action="{{ route('company.products.update', $product) }}" method="POST" style="padding: 24px;">
        @csrf
        @method('PUT')
        ...
        <div style="margin-bottom: 20px;">
            ...
            <input type="text" id="name" name="name" value="{{ old('name', $product->name) }}" required placeholder="例：シャンプー 500ml" style="
                width: 100%;
                padding: 12px 16px;
                border: 1px solid #e8e8e8;
                border-radius: 12px;
        ...
        <div style="margin-bottom: 20px;">
            ...
            <textarea id="description" name="description" rows="5" placeholder="商品の説明を入力してください" style="
                ...
                box-sizing: border-box;
                resize: vertical;
            " onfocus="this.style.borderColor='#90AFC5'; this.style.background='#ffffff';" onblur="this.style.borderColor='#e8e8e8'; this.style.background='#fafafa';">{{ old('description', $product->description) }}</textarea>
            @error('description')
                <span style="display: block; margin-top: 6px; color: #763626; font-size: 12px;">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div style="margin-bottom: 20px;">
            ...
            <input type="number" id="price" name="price" value="{{ old('price', $product->price) }}" required min="0" placeholder="0" style="
                width: 100%;
                padding: 12px 16px;
                border: 1px solid #e8e8e8;
                border-radius: 12px;
        ...
        <div style="margin-bottom: 20px;">
            ...
            <input type="number" id="stock" name="stock" value="{{ old('stock', $product->stock) }}" required min="0" placeholder="0" style="
                width: 100%;
                padding: 12px 16px;
                border: 1px solid #e8e8e8;
                border-radius: 12px;
        ...
        <div style="margin-bottom: 20px;">
            ...
            " onfocus="this.style.borderColor='#90AFC5'; this.style.background='#ffffff';" onblur="this.style.borderColor='#e8e8e8'; this.style.background='#fafafa';">
                ...
                <option value="1" {{ old('category', $product->category) == 1 ? 'selected' : '' }}>美容用品</option>
                <option value="2" {{ old('category', $product->category) == 2 ? 'selected' : '' }}>医療用品</option>
                <option value="3" {{ old('category', $product->category) == 3 ? 'selected' : '' }}>その他</option>
            </select>
            @error('category')
                <span style="display: block; margin-top: 6px; color: #763626; font-size: 12px;">{{ $message }}</span>
            @enderror
        ...
        <div style="margin-bottom: 20px;">
            ...
            <select id="status" name="status" required style="
                ...
                box-sizing: border-box;
            " onfocus="this.style.borderColor='#90AFC5'; this.style.background='#ffffff';" onblur="this.style.borderColor='#e8e8e8'; this.style.background='#fafafa';">
                <option value="1" {{ old('status', $product->status) == 1 ? 'selected' : '' }}>販売中</option>
                <option value="2" {{ old('status', $product->status) == 2 ? 'selected' : '' }}>在庫切れ</option>
                <option value="9" {{ old('status', $product->status) == 9 ? 'selected' : '' }}>非公開</option>
            </select>
            @error('status')
                <span style="display: block; margin-top: 6px; color: #763626; font-size: 12px;">{{ $message }}</span>
            @enderror
