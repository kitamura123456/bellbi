...
- **予約受付設定（事業者・店舗）**  
  - 店舗ごとに営業時間、定休日を設定できる。  
  - 予約枠は30分単位で管理する。  
  - 施術メニュー（サービスメニュー）を登録・編集できる。  
    - メニュー名、施術時間（30分単位）、料金、カテゴリ（数値）を管理する。  
  - スタッフ情報を登録・編集できる。  
    - スタッフ名、担当可能メニュー、勤務シフトなどを管理する。

- **予約カレンダー・空き枠管理**  
  - 日別・スタッフ別で予約状況を表示できる。  
  - 30分単位で予約枠の空き状況を自動計算する。  
  - 事業者側で予約枠を手動ブロック（休憩時間、貸切など）できる。

- **予約機能（個人ユーザ）**  
  - 店舗一覧・検索画面から予約可能な店舗を探せる。  
  - 店舗詳細画面からメニューを選択できる。  
  - 希望日時と空き枠を確認し、予約を確定できる。  
  - スタッフ指名は任意（指名なしの場合は店舗側でアサイン）。  
  - 予約ステータスは数値で管理する。  
    - 1: 予約確定  
    - 2: 来店済  
    - 3: 店舗キャンセル  
...
- **予約管理（事業者）**  
  - 予約一覧を日別・スタッフ別・ステータス別で閲覧できる。  
  - 予約詳細を確認し、ステータスを変更できる。  
  - 来店確認、キャンセル処理を行える。  
  - 予約時にメモを追加できる（顧客の要望、アレルギー情報など）。

- **予約履歴（個人ユーザ）**  
  - 過去の予約履歴を一覧表示できる。  
  - 予約のキャンセル（キャンセルポリシーに応じて制限）ができる。  
  - 予約変更は将来拡張として、当初はキャンセル後に再予約する運用とする。

- **キャンセルポリシー**  
  - 店舗ごとにキャンセル可能期限を設定できる（例：前日まで、3日前まで等）。  
  - キャンセル期限を過ぎた予約のキャンセルは店舗への連絡が必要となる旨を表示する。
